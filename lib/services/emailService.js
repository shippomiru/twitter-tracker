...
async function sendEmailNotification(notificationContent) {
  try {
    const { username, tweetId, originalText, translatedText, analysis, createdAt, emailAddress } = notificationContent;
    ...
    // 确定收件人，优先使用传入的emailAddress
    const to = emailAddress || process.env.EMAIL_TO;
    
    // 记录详细的邮箱检查信息
    log(`检查邮箱设置 - 传入的emailAddress: ${emailAddress || '无'}, 环境变量EMAIL_TO: ${process.env.EMAIL_TO || '无'}`);
    
    // 检查是否有有效的收件人邮箱
    if (!to) {
      log('没有配置任何收件人邮箱，跳过邮件发送', 'warn');
      return false;
    }
    
    // 检查是否是示例邮箱
    if (to === 'recipient@example.com' || to.includes('example.com')) {
      log('使用了示例邮箱，API会拒绝发送，跳过发送', 'warn');
      return false;
    }

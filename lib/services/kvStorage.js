...
class KVStorageService {
  ...
  async getSystemState() {
    try {
      const state = await kv.get(this.SYSTEM_STATE_KEY);
      if (state) {
  ...
  async saveSystemState(state) {
    try {
      ...
      await kv.set(this.SYSTEM_STATE_KEY, this.systemState);
      log('系统状态已保存', 'info');
    } catch (error) {
  ...
  async getAllUsers() {
    try {
      const users = await kv.get(this.USERS_KEY);
  ...
  async saveUserTweets(username, tweets) {
    try {
      const tweetsData = await kv.get(this.TWEETS_KEY) || {};
      tweetsData[username] = tweets;
      await kv.set(this.TWEETS_KEY, tweetsData);
